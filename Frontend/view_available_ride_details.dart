...
class ViewAvailableRideDetails extends StatefulWidget {
  final Map<String, dynamic> ride;

  const ViewAvailableRideDetails({super.key, required this.ride});

  @override
  State<ViewAvailableRideDetails> createState() =>
      _ViewAvailableRideDetailsState();
}

class _ViewAvailableRideDetailsState extends State<ViewAvailableRideDetails> {
  ...
  void initState() {
    ...
    _setupMapMarkers();
    _fetchRoute();
    _fetchDriverName(widget.ride['user_id']);
  ...
  Future<void> _fetchDriverName(String driverId) async {
    ...
    try {
      ...
      if (response.containsKey('user_name')) {
        setState(() {
          _driverName = response['user_name'] ?? "--";
          isLoading = false;
        });
      }
    } catch (e) {
                ...
                Expanded(
                  child: ListView(
                    ...
                    children: [
                      ...
                      Text(
                        Utils.formatDate(widget.ride['departure_time']),
                        style: const TextStyle(
